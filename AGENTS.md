...
- Tooling/config lives at the repo root: `package.json`, `tsconfig.json`, `tsdown.config.ts`, `.oxfmtrc.json`, `.oxlintrc.json`.
...
- `bun test`: run all tests (Vitest test runner).
- `bun test lib/core/document.test.ts`: run a single test file.
...
- Tests use Bun's Jest-compatible test runner (`bun test` with `describe`/`test`/`expect`) and live in `lib/**/**/*.test.ts`.
